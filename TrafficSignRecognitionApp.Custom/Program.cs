...
using System.IO;
using System.Linq;
using CustomVisionOnnx;
using Microsoft.ML;
using Microsoft.ML.Transforms.Image;
using TrafficSignRecognitionApp.Custom.DataStructures;

namespace TrafficSignRecognitionApp.Custom
{
    class Program
    {
        ...
        {
            @"C:\Users\Vitaly\source\repos\TrafficSignRecognitionApp\TrafficSignRecognitionApp.Custom\test\pedestrian-crossing.jpg",
            @"C:\Users\Vitaly\source\repos\TrafficSignRecognitionApp\TrafficSignRecognitionApp.Custom\test\stop.jpg"
        };
        ...
        static void Main(string[] args)
        {
            ...
            var emptyData = new List<TrafficSignInput>();

            var data = context.Data.LoadFromEnumerable(emptyData);

            //var classificationPipeline = context.Transforms.ResizeImages(
            //        resizing: ImageResizingEstimator.ResizingKind.Fill,
            //        outputColumnName: "data",
            //        imageWidth: ClassificationImageSettings.imageWidth,
            //        imageHeight: ClassificationImageSettings.imageHeight,
            //        inputColumnName: nameof(TrafficSignInput.Image))
            //    .Append(context.Transforms.ExtractPixels(outputColumnName: "data"))
            //    .Append(context.Transforms.ApplyOnnxModel(
            //        outputColumnNames: new[] { "classLabel", "loss" },
            //        inputColumnNames: new[] { "data" },
            //        modelFile: @"C:\Users\Vitaly\source\repos\TrafficSignRecognitionApp\TrafficSignRecognitionApp.Custom\ClassificationModel\model.onnx"));
            //var classificationModel = classificationPipeline.Fit(data);
            //var classificationPredictionEngine = context.Model.CreatePredictionEngine<TrafficSignInput, TrafficSignClassification>(classificationModel);

            var detectionPipeline = context.Transforms.ResizeImages(
                    resizing: ImageResizingEstimator.ResizingKind.Fill,
                    outputColumnName: "data",
                    imageWidth: DetectionImageSettings.imageWidth,
                    imageHeight: DetectionImageSettings.imageHeight,
                    inputColumnName: nameof(TrafficSignInput.Image))
                .Append(context.Transforms.ExtractPixels(outputColumnName: "data"))
                .Append(context.Transforms.ApplyOnnxModel(modelFile: @"C:\Users\Vitaly\source\repos\TrafficSignRecognitionApp\TrafficSignRecognitionApp.Custom\DetectionModel\model.onnx", outputColumnName: "model_outputs0", inputColumnName: "data"));
            var detectionModel = detectionPipeline.Fit(data);
            var detectionPredictionEngine = context.Model.CreatePredictionEngine<TrafficSignInput, TrafficSignDetection>(detectionModel);

            var labels = File.ReadAllLines(@"C:\Users\Vitaly\source\repos\TrafficSignRecognitionApp\TrafficSignRecognitionApp.Custom\DetectionModel\labels.txt");
            ...
            foreach (var image in testFiles)
            {
                ...
                //var classificationPrediction = classificationPredictionEngine.Predict(new TrafficSignInput { Image = testImage });

                //var label = classificationPrediction.Labels[0];
                //var score = classificationPrediction.Scores.First()[classificationPrediction.Labels[0]];

                var detectionPrediction = detectionPredictionEngine.Predict(new TrafficSignInput { Image = testImage });

                var boundingBoxes = ParseOutputs(detectionPrediction.PredictedLabels, labels);
                ...
                foreach (var boundingBox in boundingBoxes)
                {
                    ...
                    x = originalWidth * x / DetectionImageSettings.imageWidth;
                    y = originalHeight * y / DetectionImageSettings.imageHeight;
                    width = originalWidth * width / DetectionImageSettings.imageWidth;
                    height = originalHeight * height / DetectionImageSettings.imageHeight;
        ...
        {
            const float cellWidth = DetectionImageSettings.imageWidth / columnCount;
            const float cellHeight = DetectionImageSettings.imageHeight / rowCount;
